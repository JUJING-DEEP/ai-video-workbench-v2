...
The current Jimeng integration is a workflow-testing scaffold. It validates the
local provider lifecycle for video jobs, including provider settings, job
submission, polling, result download, local asset creation, shot binding, and
...
- Simulate result downloads.
- Bind completed outputs to the asset library and shot video path.

The real provider is out of scope for PR 1 and must not be connected until the
real integration prerequisites are satisfied.

Real provider work, when allowed by a later PR, must use the existing `jimeng`
provider identity and lifecycle. It must not add a new provider, new API, new
database table, or new frontend page.
...
- Provider `completed` proceeds to download before local completion.
- Provider timeout maps to local `timeout`.
- Provider error maps to local `failed`.

### Download

Download retrieves the completed video result after the provider reports a
successful terminal state.

Expected local behavior:

- Download must complete before the local job becomes `completed`.
- Downloaded video bytes are written under the existing project generated-video
  storage convention.
- The downloaded file path is recorded as a video asset.
- Failed downloads must not create completed jobs or bind partial outputs.

### Bind

Bind happens only after download and asset creation succeed.

Expected local behavior:

- Create a `video` asset with `source: "jimeng"`.
- Bind the asset path to the shot video path.
...
```json
{
  ...
  "data": {
    "job": {
      ...
      "output_path": "data/uploads/1/generated/videos/jimeng-rest-job-501.mp4",
      "error_message": ""
    }
  }
}
...
| `completed` | Download, asset creation, and shot binding succeeded. | No | Yes |
...
- Submit, poll, completed, failed, timeout, and download responses are observed.
- One short controlled job can be submitted, polled, downloaded, stored, bound,
  and exported into a render plan.
